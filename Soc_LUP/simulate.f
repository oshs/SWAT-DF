...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      ! === VARIABLES FOR SNAPSHOT & RESTORE baseline status ===
      real*8, allocatable, save :: base_hru_fr(:), base_hru_km(:)
      
      ! Pond & Wetland Backups
      real*8, allocatable, save :: base_pnd_psa(:), base_pnd_esa(:)
      real*8, allocatable, save :: base_pnd_pvol(:), base_pnd_evol(:), base_pnd_vol(:)
      real*8, allocatable, save :: base_wet_nsa(:), base_wet_mxsa(:)
      real*8, allocatable, save :: base_wet_nvol(:), base_wet_mxvol(:), base_wet_vol(:)
      
      ! Monthly Backups (2D Arrays)
      real*8, allocatable, save :: base_wupnd(:,:), base_wushal(:,:), base_wudeep(:,:)
      
      integer, save :: baseline_captured = 0         
      ! ============   SWAT_DF =========================
      ...
      do curyr = 1, nbyr
        ...
        ! ============   SWAT_DF =========================
        ! ==========================================================
        ! === INSERT B: RESTORE HRU BASELINE (Clean Slate) ===
        ! ==========================================================
        if (baseline_captured == 0) then
            ! --- YEAR 1: SNAPSHOT ---
            ! Allocate ALL backup arrays
            allocate(base_hru_fr(nhru), base_hru_km(nhru))
            allocate(base_pnd_psa(nhru), base_pnd_esa(nhru))
            allocate(base_pnd_pvol(nhru), base_pnd_evol(nhru), base_pnd_vol(nhru))
            allocate(base_wet_nsa(nhru), base_wet_mxsa(nhru))
            allocate(base_wet_nvol(nhru), base_wet_mxvol(nhru), base_wet_vol(nhru))
            allocate(base_wupnd(12,nhru), base_wushal(12,nhru), base_wudeep(12,nhru))
            
            
            do j = 1, nhru
                base_hru_fr(j) = hru_fr(j)
                base_hru_km(j) = hru_km(j)
                
                base_pnd_psa(j)  = pnd_psa(j)
                base_pnd_esa(j)  = pnd_esa(j)
                base_pnd_pvol(j) = pnd_pvol(j)
                base_pnd_evol(j) = pnd_evol(j)
                base_pnd_vol(j)  = pnd_vol(j)

                base_wet_nsa(j)   = wet_nsa(j)
                base_wet_mxsa(j)  = wet_mxsa(j)
                base_wet_nvol(j)  = wet_nvol(j)
                base_wet_mxvol(j) = wet_mxvol(j)
                base_wet_vol(j)   = wet_vol(j)

                do mon = 1, 12
                    base_wupnd(mon,j)  = wupnd(mon,j)
                    base_wushal(mon,j) = wushal(mon,j)
                    base_wudeep(mon,j) = wudeep(mon,j)
                end do
            end do
            
            baseline_captured = 1
            write(soclup_log_unit,*) 'Snapshot: Baseline variables saved.'

        else
            ! --- YEAR 2+: RESTORE ---
            ! Copy back the CLEAN values.
            do j = 1, nhru
                hru_fr(j) = base_hru_fr(j)
                hru_km(j) = base_hru_km(j)
                
                ! Recalculate basic geometry
                if (hru_fr(j) <= 0.0) hru_fr(j) = .0000001
                hru_ha(j)   = hru_km(j) * 100.0
                hru_dafr(j) = hru_km(j) / da_km

                ! Restore Ponds/Wetlands to baseline state
                pnd_psa(j)  = base_pnd_psa(j)
                pnd_esa(j)  = base_pnd_esa(j)
                pnd_pvol(j) = base_pnd_pvol(j)
                pnd_evol(j) = base_pnd_evol(j)
                pnd_vol(j)  = base_pnd_vol(j)

                wet_nsa(j)   = base_wet_nsa(j)
                wet_mxsa(j)  = base_wet_mxsa(j)
                wet_nvol(j)  = base_wet_nvol(j)
                wet_mxvol(j) = base_wet_mxvol(j)
                wet_vol(j)   = base_wet_vol(j)

                do mon = 1, 12
                    wupnd(mon,j)  = base_wupnd(mon,j)
                    wushal(mon,j) = base_wushal(mon,j)
                    wudeep(mon,j) = base_wudeep(mon,j)
                end do
            end do
            
            selected_lup = 0 
            write(soclup_log_unit,*) 'Restored: HRUs reset to baseline.'
        end if
        ! ============   SWAT_DF =========================
     ...
     *          sol_nh3(ly,j)
     ...
     &       drydep_nh4_d(l), drydep_no3_d(l),l=1, matmo)
           ...
           if ( curyr > nyskip .and. i_mo == 3 .and.  iida == (91 - leapyr) ) then
               !write(soclup_log_unit,*) '--- DIAGNOSTIC: March LUP Check ---'
               !write(soclup_log_unit,*) 'curyr=', curyr, ' nyskip=', nyskip, ' iyr=', iyr
               !write(soclup_log_unit,*) 'Time of check: Year=', iyr, ' Month=', i_mo
               !write(soclup_log_unit,*) 'res_vol(1) raw =', res_vol(1)/1.0e6 , 'MCM'
               
               write(soclup_log_unit,*) '--- Socio-Economic LUP Check ---'
               write(soclup_log_unit,*) 'Year = ', iyr
               write(soclup_log_unit,*) 'March 31 RES1 Volume = ', res_vol(1)/1.0e6, ' MCM'

               if (res_vol(1) > 0.) then
                   
                   res_vol_mcm = res_vol(1) / 1000000.0
                   ...
                   if (res_vol_mcm < 312.0) then
                       ...
                       write(soclup_log_unit,*) 'Selected LUP Scenario ID = ', selected_lup
                       write(soclup_log_unit,*) '-----------------------------'
                       write(soclup_log_unit,*) 'Calling resetsoclup for year ', iyr
                       call resetsoclu
                   
                       !write(soclup_log_unit,*) 'Year ', iyr, ' March Volume (RES1) = ', volume_mcm, ' MCM'
                       !write(soclup_log_unit,*) 'Selected LUP Scenario ID = ', selected_lup
                       !write(soclup_log_unit,*) 'Scenario File = ', trim(fname_soc(selected_lup))
                       write(soclup_log_unit,*) '----------------------------------------------'
                       flush(soclup_log_unit)
                   
                   else
                       ! Volume is > 312.0. Do nothing and stay in the default Scenario 1.
                       write(soclup_log_unit,*) 'Volume is high. Remaining in baseline Scenario...'
                       flush(soclup_log_unit)
                   end if
